...
fn main() {
    ...
    let mut sum = 0;
    for part in parts.iter().filter(|p| p.gear) {
        let mut neighbours = numbers.iter().filter(|number| {
            part.y.abs_diff(number.row) <= 1
                && part.x + 1 >= number.start
                && number.end + 1 >= part.x
        });

        if let (Some(n1), Some(n2), None) =
            (neighbours.next(), neighbours.next(), neighbours.next())
        {
