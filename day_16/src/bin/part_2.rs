...
fn energized_level(mut rays: Vec<Ray>, mirrors: &[Vec<Mirror>], orig_chars: &[Vec<char>]) -> usize {
...
fn process(s: &str) -> usize {
    ...
    for x in 0..width {
        ...
        maximum = maximum.max(energized_level(rays, &mirrors, &orig_chars));
        let rays = vec![Ray {
            dir: Up,
            pos: (x as i32, height as i32),
        }];
        maximum = maximum.max(energized_level(rays, &mirrors, &orig_chars));
    }
    for y in 0..height {
